...
export function SessionWorkspace() {
  ...
  const openTranscript = async (id: string, message = "Viewing saved transcript.") => { setActiveId(id); setActiveSessionType("completed"); setActiveSegment(-1); try { const data = await api.getTranscript(id); setTranscript(data); setStatus(message); } catch (error) { setStatus(error instanceof Error ? error.message : "Could not open transcript."); } };
    ...
    <main className="mx-auto w-[min(92vw,1080px)] py-8 text-center">
      <section className="mb-6 overflow-hidden rounded-2xl border border-stone-200 bg-stone-50 text-left shadow-sm">
        ...
        <nav className="flex gap-1 overflow-x-auto border-t border-stone-200 bg-white px-3 py-2" aria-label="Elena Sadić client navigation">
          ...
          <button type="button" aria-disabled="true" title="Client insights are coming soon" className={`${clientNavItemClass} cursor-not-allowed font-medium text-stone-500`}><InsightsIcon />Insights <span className="rounded-full bg-stone-100 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-stone-500">Soon</span></button>
