...
export function TranscriptViewer({ transcript, activeSegment, isPlaying, onPlaySegment, onAudioTimeUpdate, onAudioPlay, onAudioPause, onAudioEnded }: Props) {
  ...
  const summary = transcript ? (transcript.segments.length ? `${transcript.segments.length} timestamped segments` : "No speech detected") : "";
  ...
  return (
    <section className="w-full rounded-2xl border border-stone-200 bg-white p-5 shadow-sm" aria-label="Associated materials">
      ...
      <section className="relative mt-5 overflow-hidden rounded-xl border border-stone-200 bg-stone-100 text-stone-900 transition duration-200 ease-out hover:bg-stone-200" aria-label="Transcript">
        {!expanded ? (
      ...
      {transcript.recording_url ? <audio ref={audio} className="sr-only" src={transcript.recording_url} onLoadedMetadata={(event) => setDuration(event.currentTarget.duration)} onTimeUpdate={(event) => { setCurrentTime(event.currentTarget.currentTime); onAudioTimeUpdate(event); }} onPlay={onAudioPlay} onPause={onAudioPause} onEnded={onAudioEnded} /> : null}
    </section>
  );
}
